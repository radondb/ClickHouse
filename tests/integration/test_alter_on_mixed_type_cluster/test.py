...
node2 = cluster.add_instance('node2', main_configs=['configs/remote_servers2.xml'], with_zookeeper=True)
node3 = cluster.add_instance('node3', main_configs=['configs/remote_servers3.xml'], with_zookeeper=True)
node4 = cluster.add_instance('node4', main_configs=['configs/remote_servers4.xml'], with_zookeeper=True)
...
ENGINE = ReplicatedMergeTree('/clickhouse/tables/{shard}/sometable', '{replica}') ORDER BY id;
            ''')
        node3.query('''CREATE TABLE test_table(date Date, id UInt32, value Int32) ENGINE=MergeTree ORDER BY id''')
...
def test_alter_replicated_on_cluster(started_cluster):
    ...
    node1.query("ALTER TABLE test_table_replicated MODIFY COLUMN date DateTime", settings={"replication_alter_partitions_sync": "2"})
    ...
    node3.query("ALTER TABLE test_table_replicated MODIFY COLUMN value String", settings={"replication_alter_partitions_sync": "2"})

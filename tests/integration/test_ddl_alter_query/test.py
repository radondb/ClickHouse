...
node1 = cluster.add_instance('node1', main_configs=['configs/remote_servers1.xml'], with_zookeeper=True)
...
def started_cluster():
    try:
        cluster.start()
        node3.query("CREATE DATABASE testdb")
        node1.query(
            '''CREATE TABLE testdb.test_table(id UInt32, val String) ENGINE = ReplicatedMergeTree('/clickhouse/test/{shard}', '{replica}') ORDER BY id;''')
